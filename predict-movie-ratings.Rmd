...
# train matrix
train.x <- edx %>% select(-timestamp, - title, -genres, -datetime, -rating) 
...
```{r}
xgb <- readRDS("xgb.rds")
meta_user <- edx %>% select(userId, mean_user, n_movies) %>% distinct
meta_movie <- edx %>% select(movieId, releaseyear, n_genres, mean_genre, mean_movie, n_users) %>% distinct
